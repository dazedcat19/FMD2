...
local function GetAltTitles(x)
	...
	if alttitle == '' then 
		alttitle = x.XPathString('//span[@class="alternative"]')
	end
	return alttitle
end
...
local function GetCoverLink(x)
	...
	if img == '' then
		img = x.XPathString('//div[@itemprop="image"]//img/@data-src')
	end
	if img == '' then
...
local function GetAuthors(x)
	...
	if authors == '' then
		authors = x.XPathString('//div[@class="fmed" and contains(b, "' .. XPathTokenAuthors .. '")]/span')
	end
	if authors == '' then
		authors = x.XPathString('(//div[@class="imptdt" and contains(., "' .. XPathTokenAuthors .. '")]/i)[1]')
	end
	return authors
end
...
local function GetArtists(x)
	...
	if artists == '' then
		artists = x.XPathString('//div[@class="fmed" and contains(b, "' .. XPathTokenArtists .. '")]/span')
	end
	if artists == '' then
		artists = x.XPathString('(//div[@class="imptdt" and contains(., "' .. XPathTokenArtists .. '")]/i)[1]')
	end
	return artists
end
...
local function GetGenres(x)
	...
	if genre == '' then
		genre = x.XPathStringAll('//span[@class="mgen"]/a')
	end
	return genre
end
...
local function GetStatus(x)
	...
	if status == '' then
		status = x.XPathString('//div[@class="imptdt" and contains(., "' .. XPathTokenStatus .. '")]/i')
	end
	return status
end
